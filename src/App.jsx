...
const socket = io("http://192.168.0.38:3000"); // Replace with your IP
...
function App() {
  ...
  if (!isLoggedIn) {
    return (
      <motion.div
        className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-200 to-blue-100"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      >
        <motion.form
          onSubmit={handleLogin}
          className="bg-white p-8 rounded-2xl shadow-xl w-80 text-center space-y-4"
  ...
  return (
    ...
    >
      <motion.div
        className="w-full max-w-2xl bg-white shadow-xl rounded-2xl p-6 border border-indigo-200"
      ...
      >
        ...
        <div className="text-right text-sm text-gray-500 mb-2">
          Connected Clients:{" "}
          <span className="font-semibold text-indigo-600">{clientsTotal}</span>
        </div>

        <div
          ref={messageContainerRef}
          className="h-80 overflow-y-auto bg-gray-50 rounded p-3 mb-4 scroll-smooth"
        >
            ...
            {messages.map((msg, i) => (
              ...
              >
                <motion.div
                  ...
                  className={`rounded-xl px-4 py-2 text-sm shadow-lg max-w-xs ${
                    msg.isOwn
          ...
          {feedback && (
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-sm italic text-purple-500 mt-1"
            >
              {feedback}
            </motion.p>
          )}
        ...
        <form onSubmit={sendMessage} className="flex gap-2 items-center">
          <motion.input
            ...
            className="flex-1 p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow shadow-inner"
            value={message}
